...
def atribuiPesos(matriz, minPeso, maxPeso):
    for i in range(len(matriz)):
        for j in range(len(matriz[i])):
            if matriz[i][j] > 0:
                matriz[i][j] = sum([random.randint(minPeso, maxPeso) for _ in range(matriz[i][j])])
    return matriz
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for i, dataset in enumerate(datasets):
            nomeArq = (
                f"{tipos[tipo]}-{geracao[fator][0]}-{numV}-{numA}-{seed}-{i+1}-{numComp}"
            )
