...
def geraGrafoPwl(numV, numA, gamma=2.5, dirigido=False, tipo=0, seed=None):
    ...
    soma_desejada = 2 * numA if not dirigido else numA
    graus = geraGrausPwl(numV, soma_desejada, gamma)
...
if __name__ == "__main__":
    ...
    gamma = float(input("Expoente (gamma) da distribuição (padrão 2.5): ") or 2.5) #aleatório entre 2 e 3
    seed = input("Semente (opcional): ")
    seed = int(seed) if seed.strip() != "" else None #garantir reprodutibilidade

    arestas, G = geraGrafoPwl(numV, numA, gamma, dirigido, tipo, seed)
    # print("Arestas geradas:", arestas)
